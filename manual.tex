...
Cieľom tohto projektu je vytvoriť prenositeľnú konzolovú aplikáciu, ktorá pomocou existujúceho "bot" účtu posiela echo odozvy na všetky správy, ktoré zaznamená v textovom kanáli \texttt{isa-bot}. Spomínaný "bot" účet, musí byť vytvorený na platforme Discord \cite{discord} a musí byť autorizovaný v nejakom servere, ktorý je takisto vytvorený na platforme Discord \cite{discord}. Postup vytvorenia a autorizovania "bot" účtu je popísaný na tejto adrese~\cite{creating_a_bot}. Detaily k nastaveniu "bot" účtu sú popísané v kapitole \ref{4:1}.
...
Podobne je to z textovým kanálom \texttt{isa-bot}. Takisto nie je špecifikované koľko kanálov s týmto menom môže byť vytvorených v jednom Discord \cite{discord} servere. Aplikácia akceptuje také "bot" účty, ktoré sú autorizované v servere s presne jedným takto nazvaným textovým kanálom.
...
Ako náhle je zaručené, že boli prečítané nejaké dáta z SSL spojenia, skontroluje sa, či server nevrátil chybový HTTP kód. Môže nastať jeden z nasledujúcich stavov:
\begin{enumerate}
...
\begin{table}[ht]
	...
	\begin{tabular}{lll}
		...
		v servere neexistuje text kanál s názvom "isa-bot"          & EXIT\_FAILURE              & there is no "isa-bot" channel in this Discord guild                         \\
		v servere existuje viac ako 1 text kanál s názvom "isa-bot" & EXIT\_FAILURE              & there is more than 1 "isa-bot" channel in this Discord guild                \\
...
Program ukončí svoju činnosť, keď zachytí signál \texttt{SIGINT} (napríklad po stlačení kláves \texttt{ctrl + c}).
